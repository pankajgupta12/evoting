<?php  
$result = $this->db->get_where('tbl_position',array('status'=>1))->result();
if(!empty($result)){
			...
			<div class="container-fluid">
				<div class="row">
					<div class="col-md-12">
						...
						<div class="content">
						<?php 
						 /* $candidtelist=$this->db->select('*')->from('tbl_uer_positionvoting')->where(array('user_id'=>$this->session->userdata('user_id')))->get()->result(); 
						...
						?>
								<form action="<?php echo base_url('user/user_voting'); ?>" id="user_voting" method="post">                                
							    	...
							    	$getpositionName = $this->db->select('*')->get_where('tbl_position',array('position_id'=>$postlist->position_id,'status'=>1))->row(); 
									if(!empty($getpositionName)){
									
									$candidte_postlist=$this->db->select('*')->from('tbl_uer_positionvoting')->where(array('user_id'=>$this->session->userdata('user_id'),'	position_id'=>$postlist->position_id))->get()->row();
									if(empty($candidte_postlist)){	
									
										?>
										<div class="row" >
											<div class="col-md-6">
												<span><h5><?php echo $i+1;?>. <?php echo $getpositionName->position_name; ?></h5></span>
											</div>
								<?php // print_r($postlist);  ?>			
									<input type="hidden" name="position_id[<?php echo $i;?>]" value="<?php echo $postlist->position_id; ?>">		
											<div class="col-md-6 form-group">
												 <select name="candidate_id1[<?php echo $i;?>]" id="candidate_id" class="form-control" style="width:70%">
														<option value="" style="width:70%">Select Candidate</option>
														<?php foreach($candidtelist as $candidtelist){ ?>
														<option value="<?php echo $candidtelist->candidate_id; ?>"<?php if(1){ echo set_select('candidate_id1['.$i.']',$candidtelist->candidate_id); }  ?> style="width:70%"><?php echo $candidtelist->candidate_name; ?></option>
														<?php } ?>
												</select>
													<span class="error" style="color:red;"><?php echo form_error('candidate_id1['.$i.']'); ?></span>
											</div>
										</div>
